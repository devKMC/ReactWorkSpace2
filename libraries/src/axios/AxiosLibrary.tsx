...
export default function AxiosLibrary() {
    ...
    axios.get('http://localhost:4100/api/v1/user/email@email.com')

        // 각각의 http method 함수 결과를 then으로 받을 땐 response 객체를 매개변수로 받는 콜백 함수를 전달
        .then(response => {
            // RESPONSE 객체는 STATUS, header, data(body)를 포함하고 있음
            // then 작업은 status code가 400, 500번대가 아닌 결과를 받아옴
            console.log(response)
        })
        .catch(error => {
            // 해당 request에 대해서 request가 실패하거나 status code가 400, 500 번대이면 catch 로 AxiosError 객체를 받아옴
            // response가 존재하는 에러는 error 객체에 response가 포함되어 있음
            console.log(error);
        })

    // option 매개변수
    // -request의 각종 설정(header)을 조작할 때 사용됨
    axios.get(GET_URL, { headers: { 'Authorization': 'Bearer 123456789' }, params: { 'time': '1144' } })

    // request body 매개변수
    // - post, patch, put 함수는 두번째 매개변수로 request body 데이터를 전달함 ( 이 3가지는 BODY로 받아와야함)
