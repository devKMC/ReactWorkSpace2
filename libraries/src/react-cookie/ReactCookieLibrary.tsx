...
// - react 쿠키를 쉽게 관리할 수 있도록 도움을 주는 라이브러리
...
export default function ReactCookieLibrary() {
    ...
    const onRegistButtonClickHandler = () => {
        ...
        // 방법 1
        const expires = now.setSeconds(now.getSeconds() + 10)
        // setCookie 함수는 첫번째 매개변수로 name 값 / 두번째 매개변수로 value 값 / 새번째 매개변수로 option 지정
        // option : -path : 쿠키를 사용할 수 있는 경로
        //        : -expires : 쿠키 만료 시간
        setCookie(key, value, { path: '/', expires: new Date(expires) });
    ...
    const onRemoveButtonClickHandler = () => {
        // removeCookie 함수는 첫번째 매개변수 name 값 / 두번째 매개변수로 option을 지정
        removeCookie(key);

    }
    return (
        <div>
            <input onChange={onKeyChangeHandler} />
            <input onChange={onValueChangeHandler} />
            <button onClick={onButtonClickHandler}>쿠키확인</button>
            <button onClick={onRegistButtonClickHandler}>쿠키등록</button>
            <button onClick={onRemoveButtonClickHandler}>쿠키삭제</button>
        </div>
    )
}
